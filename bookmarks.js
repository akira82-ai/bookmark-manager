...
class BookmarkManager {
  constructor() {
    ...
    // 智能书签提醒功能
    this.reminderEnabled = true; // 启用提醒功能
    ...
    this.activeReminder = null; // 当前活动的提醒
    this.reminderSettings = {
      domainThreshold: 5, // 域名触发阈值
      urlThreshold: 3, // URL触发阈值
      domainTimeWindow: 7 * 24 * 60 * 60 * 1000, // 7天
      urlTimeWindow: 3 * 24 * 60 * 60 * 1000, // 3天
      minStayTime: 2 * 60 * 1000, // 最小停留时间2分钟
      excludedDomains: ['google.com', 'bing.com', 'baidu.com', 'weixin.qq.com', 'mail.qq.com']
    };
  ...
  init() {
    ...
    // 初始化智能提醒功能
    this.initializeReminderFeature();
  }
...
createSearchResultCard(bookmark) {
  ...
  initializeReminderFeature() {
    ...
    chrome.storage.local.get(['reminderSettings'], (result) => {
      if (result.reminderSettings) {
        this.reminderSettings = { ...this.reminderSettings, ...result.reminderSettings };
      }
    });
    ...
    chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
      if (changeInfo.status === 'complete' && tab.url) {
        this.recordPageAccess(tab.url, tab.title);
      }
    });

    // 定期检查提醒条件
    setInterval(() => {
      this.checkReminderConditions();
    }, 30000); // 每30秒检查一次
  }
   ...
   * 记录页面访问
   */
  recordPageAccess(url, title) {
    if (!this.reminderEnabled || !url) return;

    const domain = this.extractMainDomain(url);
    
    // 检查是否在排除列表中
    if (this.reminderSettings.excludedDomains.includes(domain)) return;

    const now = Date.now();
    
    // 记录域名访问
    if (!this.domainAccessData.has(domain)) {
      this.domainAccessData.set(domain, []);
    }
    this.domainAccessData.get(domain).push({ time: now, url, title });

    // 记录URL访问
    if (!this.urlAccessData.has(url)) {
      this.urlAccessData.set(url, []);
    }
    this.urlAccessData.get(url).push({ time: now, title });
  ...
  checkReminderConditions() {
    ...
    for (const [domain, visits] of this.domainAccessData) {
      if (this.dismissedDomains.has(domain)) continue;
      
      const snoozeUntil = this.snoozedDomains.get(domain);
      if (snoozeUntil && now < snoozeUntil) continue;

      const recentVisits = visits.filter(v => now - v.time < this.reminderSettings.domainTimeWindow);
      if (recentVisits.length >= this.reminderSettings.domainThreshold) {
        this.showDomainReminder(domain, recentVisits);
        break;
      }
    }
    ...
    for (const [url, visits] of this.urlAccessData) {
      if (this.dismissedUrls.has(url)) continue;
      
      const snoozeUntil = this.snoozedUrls.get(url);
      if (snoozeUntil && now < snoozeUntil) continue;

      const recentVisits = visits.filter(v => now - v.time < this.reminderSettings.urlTimeWindow);
      if (recentVisits.length >= this.reminderSettings.urlThreshold) {
        this.showUrlReminder(url, recentVisits);
        break;
      }
    }
  }
   ...
   * 显示域名级别提醒
   */
  showDomainReminder(domain, visits) {
    if (this.activeReminder) return;

    const latestVisit = visits[visits.length - 1];
    const reminder = document.createElement('div');
    reminder.className = 'bookmark-reminder';
    reminder.innerHTML = `
      <div class="reminder-content">
        <div class="reminder-icon">🔖</div>
        <div class="reminder-text">
          <div class="reminder-title">添加书签提醒</div>
          <div class="reminder-message">您最近访问了 ${domain} ${visits.length} 次，是否要添加书签？</div>
        </div>
        <div class="reminder-actions">
          <button class="reminder-btn save-btn">收藏</button>
          <button class="reminder-btn snooze-btn">稍后提醒</button>
          <button class="reminder-btn dismiss-btn">不再提醒</button>
        </div>
      </div>
    `;

    document.body.appendChild(reminder);
    this.activeReminder = { type: 'domain', data: domain, element: reminder };
    ...
    // 绑定事件
    reminder.querySelector('.save-btn').onclick = () => this.addDomainBookmark(domain, latestVisit.title);
    reminder.querySelector('.snooze-btn').onclick = () => this.snoozeReminder('domain', domain);
    reminder.querySelector('.dismiss-btn').onclick = () => this.dismissReminder('domain', domain);
  }

  /**
   * 显示URL级别提醒
   */
  showUrlReminder(url, visits) {
    if (this.activeReminder) return;
    ...
    reminder.innerHTML = `
      <div class="reminder-content">
        ...
        <div class="reminder-text">
          <div class="reminder-title">添加书签提醒</div>
          <div class="reminder-message">您最近多次访问了这个页面，是否要添加书签？</div>
        </div>
        <div class="reminder-actions">
          <button class="reminder-btn save-btn">收藏</button>
          <button class="reminder-btn snooze-btn">稍后提醒</button>
    ...
    document.body.appendChild(reminder);
    this.activeReminder = { type: 'url', data: url, element: reminder };
    ...
    // 绑定事件
    reminder.querySelector('.save-btn').onclick = () => this.addUrlBookmark(url, latestVisit.title);
    reminder.querySelector('.snooze-btn').onclick = () => this.snoozeReminder('url', url);
    reminder.querySelector('.dismiss-btn').onclick = () => this.dismissReminder('url', url);
  }
